...
if [ ! -f /etc/php/8.2/fpm/pool.d/www.conf ]; then
  echo "ERROR: php-fpm pool config not found: /etc/php/8.2/fpm/pool.d/www.conf" >&2
  echo "Did you COPY tools/www.conf in the Dockerfile?" >&2
  exit 1
fi
...
if [ "${1#php-fpm}" != "$1" ]; then
  echo "Validating PHP-FPM config: $1 -tt"
  "$1" -tt
fi
